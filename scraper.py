                        ...
                        for link in soup.find_all('a'):
                            ...
                            if (final not in scraper_data.visited) and (final not in new_urls):
                                ...
                                for valid_url_domain in url_set:
                                    ...
                                    if valid_url_domain in final_domain:
...
def isValidURL(url):
    # Information we don't want, logins, downloads, calendars, etc.
    invalid_queries = ['ical=', 'mailto:', 'image=', '=download', '=edit', '=login', 'replytocom=', '.calendar.']
    for invalid_query in invalid_queries:
        if invalid_query in url:
            return True

    # url has no repeating path
    if urlRepeatsPaths(url):
        return True

    return False

# Checks for similar pages
...
def subdomain_check(url):
    ...
    if "ics.uci.edu" in url:
        ...
        if subdomain and subdomain != 'www':
            ...
            scraper_data.subdomains_count[key] += 1